...
public class BugReportGenerator extends Thread {
    ...
    public void run() {
        try {
            Server.getInstance().getLogger().info("BugReport | An error has been detected, creating a bug report.");
            String path = generate();
            Server.getInstance().getLogger().info("BugReport | A bug report has been created: " + path);
        } catch (Exception e) {
            StringWriter stringWriter = new StringWriter();
            e.printStackTrace(new PrintWriter(stringWriter));
            Server.getInstance().getLogger().info("BugReport | Could not create bug report: " + stringWriter);
        }
    }
