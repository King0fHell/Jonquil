...
public class PluginManager {
                    ...
                    try {
                        ...
                        if (description != null) {
                            ...
                            if (plugins.containsKey(name) || this.getPlugin(name) != null) {
                                this.server.getLogger().error("Could not load \"" + name + "\" | " + "plugin plugin exists.");
                                continue;
                            }
                            ...
                            for (String version : description.getCompatibleAPIs()) {
                                ...
                                try {
                                    ...
                                    this.server.getLogger().error("Could not load plugin \"" + name + "\" | Wrong API format.");
                                    continue;
                                }
                            ...
                            if (!compatible) {
                                this.server.getLogger().error("Could not load plugin \"" + name + "\" | Incompatible API version.");
                            }
                        ...
                        this.server.getLogger().error("Could not load \"" + file.getName() + "\" in folder " + dictionary.toString() + " | " + Utils.getExceptionMessage(e));
    ...
    protected List<PluginCommand> parseYamlCommands(Plugin plugin) {
        ...
        for (Map.Entry entry : plugin.getDescription().getCommands().entrySet()) {
            ...
            if (key.contains(":")) {
                this.server.getLogger().critical("Could not load command " + key + " for plugin " + plugin.getDescription().getFullName());
                continue;
            }
                ...
                if (((Map) data).containsKey("aliases")) {
                    ...
                    if (aliases instanceof List) {
                        ...
                        for (String alias : (List<String>) aliases) {
                            if (alias.contains(":")) {
                                this.server.getLogger().critical("Could not load alias " + alias + " for plugin " + plugin.getDescription().getFullName());
                                continue;
                            }
                            aliasList.add(alias);
                        }
        ...
        try {
            for (RegisteredListener registration : getEventListeners(event.getClass()).getRegisteredListeners()) {
                ...
                try {
                    ...
                    this.server.getLogger().critical("Could not pass event \"" + event.getEventName() + "\" to \"" + registration.getPlugin().getDescription().getFullName() + "\" | " + e.getMessage() + " on " + registration.getListener().getClass().getName());
            ...
            for (Class<?> clazz = eventClass; Event.class.isAssignableFrom(clazz); clazz = clazz.getSuperclass()) {
                ...
                if (clazz.getAnnotation(Deprecated.class) != null) {
                    if (Boolean.valueOf(String.valueOf(this.server.getConfig("settings.deprecated-verbpse", true)))) {
                        this.server.getLogger().warning("Plugin \"" + plugin.getName() + "\" has registered a listener for \"" + clazz.getName() + "\" on method \"" + listener.getClass().getName() + "." + method.getName() + "()\", but the event is Deprecated.");
                    }
                    break;
                }
            }
